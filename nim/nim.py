...
class NimAI():
    ...
    def get_q_value(self, state, action):
        ...
        # if no reward assigned to (state, action), return 0
    ...
    def best_future_reward(self, state):
        ...
        # if no available action, return 0
    ...
    def choose_action(self, state, epsilon=True):
        ...
        # if True, choose an action with probability epsilon
        if epsilon:
            epsilons = (len(available_actions) * [self.epsilon])
            return random.choices(list(available_actions), weights=epsilons, k=1)[0]
        else:
            q_values = []
